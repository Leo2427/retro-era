  ...
  for (const f of fs.readdirSync(dir)) {
    ...
    for (const line of content.split("\n")) {
      try {
        ...
        let en = ""
        const userMsg = parsed.message?.role === "user" ? parsed.message?.content : ""
        if (typeof userMsg === "string") {
          const match = userMsg.match(/街机游戏\s+"([^"]+)"/)
          if (match) en = match[1]
        }
        for (const item of msg) {
          if (item.type === "tool_use" && item.name === "StructuredOutput" && item.input?.cn) {
            ...
            if (i.cn !== "未找到" && i.dev && i.dev !== "未知") {
              const inputEn = i.en || i.titleEn || en
              results.push({ cn: i.cn, en: inputEn, dev: i.dev, year: parseInt(i.year) || 0, desc: i.desc || "" })
            }
          }
        }
      } catch {}
...
walkDir(WF_DIR)
const seen = new Set()
const unique = results.filter(r => { const k = r.cn + r.en; if (seen.has(k)) return false; seen.add(k); return true })
